...
async def generate_report(
    ...
    current_user: User = Depends(
        check_role(["ADMIN", "INFRASTRUCTURE_ENGINEER"])
    ),
):
    report = ReportService.generate_pdf_report(db, recon_id, current_user)
    AuditService.log_action(
        db, current_user, AuditActionType.GENERATE_REPORT, f"Report ID: {report.id}"
    )
    return {
        "id": report.id,
...
async def download_report(
    ...
    return FileResponse(
        ...
        filename=f"reconciliation_report_{report.reconciliation_id}.pdf",
    )
